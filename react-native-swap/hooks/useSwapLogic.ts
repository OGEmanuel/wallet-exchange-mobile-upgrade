...
import { SwapMetaData, SupportedCurrencyModel, SwapRateModel } from '../types';

// Mock RootState type - adjust based on your Redux store structure
interface RootState {
  swap: {
...
export const useSwapLogic = () => {
  ...
  const [swapMetaData, setSwapMetaData] = useState<SwapMetaData>(initialSwapData);
    ...
    debounce(
      async (
        ...
        try {
          ...
          dispatch(swapActions.setSwapRateError('Failed to fetch exchange rates'));
        } finally {
  ...
  const handleSellInputChange = useCallback(
    (text: string) => {
      ...
      if (swapMetaData.isDollarMode) {
        ...
        formattedValue = (isSwapped ? '' : '$') + formatNumberWithCommas(cleanValue);
      } else {
  ...
  const triggerDollarCryptoSwap = useCallback(() => {
    ...
    if (swapMetaData.isDollarMode) {
      ...
      const valueWithoutDollar = (swapMetaData.dollarValue || '').replace(/\$/g, '');
