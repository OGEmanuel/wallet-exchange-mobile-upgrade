import TokenSelectionBottomSheet from "@/components/bottomsheets/TokenSelectorBottomSheet";
...
import { useCreateOrder, useFetchCurrencies } from "@/src/modules/swap";
...
const SwapScreenCompact = () => {
  ...
  const handleContinue = useCallback(async () => {
    ...
    const isFiatCurrency =
      !sellCurrency?.currencyId?.isCrypto ||
  ...
  const rateDetails = useMemo(() => {
    if (swapRate && sellCurrency && receiveCurrency) {
      ...
      const amount =
        parseFloat(swapMetaData.receiveInputValue.replace(/,/g, "")) || 0;
      const fee = amount * 0.013; // 1.3% fee example

      return {
        rate: `1 ${
          sellCurrency.currencyId?.symbol
        } ≈ ${rate.toLocaleString()} ${receiveCurrency.currencyId?.symbol}`,
                  ...
                  <Image
                    source={{
                      uri:
                        ...
                        receiveCurrency.currencyId?.logo,
                    }}
                    style={styles.currencyImage}
                  />
                ) : null}
